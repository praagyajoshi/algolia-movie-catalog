...
The application is composed of multiple docker containers housing a `Ruby on Rails` API application, a `React` frontend application, a `Mongo` data store, and an `Nginx` reverse proxy server managing requests between them.

## Setup instructions

- Install [Docker](https://store.docker.com/editions/community/docker-ce-desktop-mac)
- Clone this repository
- Create `frontend/.env` and `backend/.env` files with your Algolia API keys by referring to `frontend/.env.sample` and `backend/.env.sample`
- Run the setup script  
  `./setup.sh`

The setup script will take care of checking relevant files, pulling Docker images, running the containers, and importing the initial dataset.
...
- **`backend` container**  
  Contains a `Ruby on Rails` based API application. It is responsible for responding to 2 API calls - `POST` `/api/v1/movies` and `DELETE` `/api/v1/movies/:id`.
- **`frontend` container**  
  Contains a `React` application, which communicates with the `backend` container and Algolia API.
- **`mongo` container**  
...
While it is possible to embed a `React` application inside a `Ruby on Rails` application, for this application, `frontend` and `backend` are kept entirely separate from each other.  
This helps in facilitating a clear API which can then be used for other clients like mobile apps. It also has the benefit of more flexibility with respect to the `React` application. The disadvantage of managing another standalone app isn't much as `Docker` takes care of a lot of complexity for us.
...
```
This created an application without on all the view logic of Rails, and instead focuses on just serving JSON resouces to API clients. It also skipped the active record integration, which was replaced by the `Mongoid` ODM since we're using `MongoDB` as the data store.
...
Delete movie | DELETE | `/api/v1/movies/:id` | Creates a movie
...
- `uploaded_image` : `Image` The poster for the movie - jpg, jpeg, gif & png files are accepted
...
Searchable attributed, attributes for faceting, attributes to highlight, unretrievable attributes, and custom ranking have all been defined, instead of relying on the web interface of Algolia. This has been done to ensure interoperatibility between different Algolia accounts.

### Gems used
Keeping `DRY` in mind, a few Gems were used to facilitate development. The major onces include:
...
- **[Mongoid](https://github.com/mongodb/mongoid)**  
  Since `MongoDB` was our choice of data store, Mongoid provides an excellent Object Document Mapper (ODM) framework for it.
...
- **[Dotenv](https://github.com/bkeepers/dotenv)**  
  It is a good idea to set configuration in environment variables. For development, Dotenv makes it easy to load variables from an environment file.
- **[RSpec Rails](https://github.com/rspec/rspec-rails)**  
  RSpec was the test suite of choice. For facilitate writing specs, [Database Cleaner](https://github.com/DatabaseCleaner/database_cleaner), [Factory Bot](https://github.com/thoughtbot/factory_bot_rails), and [Faker](https://github.com/stympy/faker) were also used.
...
To make development easier, 2 more functionalities were added:
- Hot reloading  
...
The three main data providers (API, search and state) are located in their own directory (`/frontend/src/dataProviders`), along with a few constants (`frontend/src/constants`) and utility functions (`frontend/src/utils`).
...
Search is front and centre as it's our primary method of discovery. Unlike other forms of search (for example, Amazon) where filters are reset when the search query changes, this app maintains the currently selected filters.  
To communicate this to the user, filters have been brought up to be at the same visual hierarchy level.

The "Add Movie" button is present in the header as a separate action, while the "Delete movie" button only reveals itself when a movie is hovered upon. This ensure the chances of accidently deleting a movie.

### State management

In lieu of going with something along the lines of `Redux`, this application uses the URL query parameters for state management.  
Since our primary state is going to revolve around the search query, and selected filters, using the URL allows shareable links to be generated.  
[React URL Query](https://github.com/pbeshai/react-url-query) was used to facilitate the serialising & deserialising of parameters. It is inspired by `Redux` in that we get equivalent methods to `mapStateToProps` and `mapDispatchToProps`, which makes development very easy.

### Packages used
The major `npm` packages used were:
- **[Algolia Search](https://github.com/algolia/algoliasearch-client-javascript)**  
  The JavaScript client of Algolia was used to power search on the frontend. This client was used instead of the [InstantSearch](https://github.com/algolia/react-instantsearch) React library to facilitate building completely custom UI components.
- **[React URL Query](https://github.com/pbeshai/react-url-query)**  
  To manage state using URL query parameters.
- **[Axios](https://github.com/axios/axios)**  
  The popular promise based HTTP client was used for making calls to the `backend` container.
- **[Bulma](https://github.com/jgthms/bulma)**  
  The opinionated CSS framework was used to provide styling for this application as it provides a wide variety of baked in UI components, with easy customisability.
- **[React Helmet](https://github.com/nfl/react-helmet)**  
  Allows easy manipulation of the document `<head>` attributes.

## Storage

Since we're dealing with movies which are completely independent of each other, going with a NoSQL database makes the most sense. Based on our initial data set, storing each movie as an independent document made for the best fit.  
